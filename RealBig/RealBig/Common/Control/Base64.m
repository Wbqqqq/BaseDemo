...
- (NSString *)authCode:(NSString *)auth_key operation:(NSStringAuthCode)operation encoding:(NSStringEncoding)encoding
{
    ...
    NSString * keyB = [[NSString stringWithFormat:@"%@",[key substringWithRange:NSMakeRange(16,16)]] md5];
    
    NSDate *date = [NSDate date];
    NSString * Date = [NSString stringWithFormat:@"%ld", (long)[date timeIntervalSince1970]];

    Date = [Date md5];
    NSInteger  Datelength = Date.length;
    NSString * keyC =[NSString stringWithFormat:@"%@",[Date substringWithRange:NSMakeRange(Datelength-4,4)]];
    
    //'0000000000' . substr(md5($string . $keyb), 0, 16) . $string;
    NSString * selfstr = self;
    NSString * selfstrmd5 = [[NSString stringWithFormat:@"%@%@",self,keyB] md5];
    selfstrmd5 = [selfstrmd5 substringWithRange:NSMakeRange(0,16)];
    selfstr = [NSString stringWithFormat:@"%@%@%@",@"0000000000",selfstrmd5,selfstr];
    
    NSInteger selfstr_lenght = selfstr.length;
    ...
    int j = 0;
    int a = 0;
    for(int i = 0; i < selfstr_lenght; i++) {
        a = (a + 1) % 128;
        ...
        unsigned short sc = [selfstr characterAtIndex:i];
        unsigned short bi = [[box objectAtIndex:(([[box objectAtIndex:a] unsignedShortValue] + [[box objectAtIndex:j] unsignedShortValue]) % 128)] unsignedShortValue];
    ...
    if(operation == NSStringAuthCodeDecoded) {
        ...
        return [NSString stringWithFormat:@"%@%@",keyC,[[result base64EncodedString:encoding]  stringByReplacingOccurrencesOfString:@"=" withString:@""]];
    }
}
